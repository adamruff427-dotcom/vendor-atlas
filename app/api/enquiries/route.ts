...
export async function POST(request: Request) {
  ...
  if (!id || !companyName || !contactName || !emailPattern.test(businessEmail) || input.consent !== true || input.serviceId !== 'dsear') return Response.json({ error: 'Required enquiry fields are invalid' }, { status: 422, headers: noStore })
  ...
  await db.insert(enquiries).values({ id, createdAt: new Date().toISOString(), serviceId: 'dsear', companyName, contactName, businessEmail, phone: phone || null, consent: true, payload: JSON.stringify(input), status: 'received' })
