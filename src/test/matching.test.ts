...
describe('supplier matching', () => {
  it('maintains a substantive evidence directory without fabricating missing source links', () => { expect(suppliers).toHaveLength(12); expect(new Set(suppliers.map((supplier) => supplier.id)).size).toBe(suppliers.length); suppliers.forEach((supplier) => { expect(supplier.evidence.length).toBeGreaterThan(0); expect(supplier.website.startsWith('https://')).toBe(true); expect(supplier.evidence.every((item) => item.sourceUrl.startsWith('https://') && item.checkedOn === supplier.lastVerifiedDate)).toBe(true) }) })
