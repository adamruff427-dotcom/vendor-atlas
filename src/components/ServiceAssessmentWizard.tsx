...
function ServiceResults({ serviceId, answers, result, estimate, matches, quoteOpen, onQuote }: {
  ...
  return (
    <div className="results">
      <div className={`result-banner ${result.status}`}><span className="result-status-icon" aria-hidden="true"><Icon strokeWidth={1.8} /></span><div><span className="eyebrow">Your initial indication</span><h3>{heading}</h3><p>{result.status === "no-obvious-trigger" ? (isPat ? "That is not a legal exemption. Confirm who controls the equipment and keep a risk-based maintenance regime even where combined electrical testing is not justified." : "That is not a legal exemption. Recheck the equipment, operating data and any uncertainty with a competent person before deciding that no examination is required.") : (isPat ? "Your answers identify equipment, use or record signals that justify a competent maintenance review and may justify combined inspection and testing." : "Your answers identify equipment, operating or record signals that justify system-specific competent-person review or examination.")}</p></div></div>
      ...
      <div className="disclaimer"><strong>{isPat ? "This is not electrical equipment inspection, testing or a legal decision." : "This is not the statutory examination or legal decision."}</strong> {result.caveats.join(" ")}</div>
    </div>
  );
}
